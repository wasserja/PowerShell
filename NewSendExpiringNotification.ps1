<#
.Synopsis
   Send-PasswordExpirationNotification
.DESCRIPTION
   Send-PasswordExpirationNotification
.EXAMPLE
   Send-PasswordExpirationNotification
...
function Send-PasswordExpirationNotification
{
    ...
    Process
    {
        ...
        foreach ($user in (Get-ADUser -SearchBase "DC=domain,DC=com" -Filter '' -properties mail | ? {$_.samaccountname -match "bvazquez|jwasser"}  ))
        {           
